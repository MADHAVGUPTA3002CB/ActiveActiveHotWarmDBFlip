...
test("keeps the live dashboard and safety copy in the application", async () => {
  ...
  assert.match(dashboard, /G · atomic detach \+ per-leaf marker/);
  assert.match(dashboard, /Apply live settings/);
  assert.match(dashboard, /Active target TPS/);
  assert.match(dashboard, /Retiring target TPS/);
  assert.match(dashboard, /rows \/ transaction/i);
  assert.match(dashboard, /achieved TPS/);
  assert.match(dashboard, /target TPS/);
  assert.match(dashboard, /rejected\/missed/);
  ...
  assert.match(dashboard, /legacy unreserved API batch \(superseded\)/i);
  assert.match(dashboard, /Admission stop/);
  assert.match(dashboard, /In-flight resolution/);
  assert.match(dashboard, /formatFenceExperiment\(selectedRun\)/);
  assert.match(dashboard, /next\.flip\.status === "running" \? next\.flip\.fence_wakeup_mode/);
  assert.match(dashboard, /Fence wake-up/);
  assert.match(dashboard, /Slot wait after wake-up/);
